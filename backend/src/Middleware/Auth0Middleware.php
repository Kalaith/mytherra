...
class Auth0Middleware implements MiddlewareInterface
{
    ...
    public function process(Request $request, RequestHandlerInterface $handler): Response
    {
        ...
        try {
            ...
            // Get or create user based on Auth0 data
            $user = $this->getOrCreateUser($decoded);
            
            if (!$user) {
                return $this->createUnauthorizedResponse('Failed to create or retrieve user');
            }
        ...
        try {
            ...
            if (!$user) {
                ...
                $user = User::create([
                    ...
                    'display_name' => $auth0User->name ?? $auth0User->email ?? 'Adventurer',
                    'divine_influence' => 100, // Default starting divine influence for Mytherra
                    'divine_favor' => 100, // Default starting divine favor
                    'level' => 1,
                    'experience' => 0,
                    'character_class' => 'novice', // Default character class for MMO
                    'guild_id' => null,
                    'guild_rank' => null,
                    'betting_stats' => json_encode([]),
                    'game_preferences' => json_encode([]),
                    'is_active' => true,
