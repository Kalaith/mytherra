...
class GameDataSeeder
{
     ...
     * Seed main game entities (regions, settlements, heroes, events, landmarks)
     */
    ...
    {
        echo "Creating landmarks...\n";
        
        foreach (LandmarkData::getData() as $landmarkData) {
            try {
                $landmark = new Landmark($landmarkData);
                $landmark->save();
                echo "Created landmark: {$landmark->name}\n";
            } catch (\Exception $e) {
                echo "Error creating landmark {$landmarkData['id']}: " . $e->getMessage() . "\n";
            }
        }
    }
