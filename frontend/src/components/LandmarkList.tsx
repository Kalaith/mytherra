...
const LandmarkList: React.FC<LandmarkListProps> = ({ 
  ...
  const getStatusColor = (status: Landmark['status']) => {
    switch (status) {
      ...
      case 'weathered': return 'text-yellow-400';
      case 'corrupted': return 'text-red-400';
      case 'blessed': return 'text-blue-400';
      case 'haunted': return 'text-purple-400';
        ...
        {landmarks.map((landmark) => {
          ...
          return (
            <div
              ...
              onClick={() => onSelectLandmark(
                selectedLandmark?.id === landmark.id ? null : landmark
              )}
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-xs ${dangerInfo.color}`}>
                    Danger: {dangerInfo.text}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-4 p-3 bg-gray-700 rounded">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            {selectedLandmark.discoveredYear && (
              <div>
                <span className="text-gray-400">Discovered:</span> Year {selectedLandmark.discoveredYear}
              </div>
            )}
            {selectedLandmark.lastVisitedYear && (
              <div>
                <span className="text-gray-400">Last Visited:</span> Year {selectedLandmark.lastVisitedYear}
              </div>
            )}
            <div className="col-span-2">
              <span className="text-gray-400">Events:</span> {selectedLandmark.associatedEvents.length} recorded
            </div>
          </div>
        </div>
      )}
