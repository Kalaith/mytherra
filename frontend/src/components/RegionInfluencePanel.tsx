      ...
      {selectedRegion ? (
        <div className="text-center">
          <p className="mb-3 text-lg">
            Targeting Region: <span className="font-semibold text-yellow-400">{selectedRegion.name}</span>
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <button
              onClick={() => handleInfluenceAction('Bless Region', selectedRegion.id, selectedRegion.name, 'region')}
              disabled={
            ...
            >
              ...
              {selectedRegion.influenceActionCosts?.blessRegion !== undefined && (
                <span className="text-xs"> (Cost: {selectedRegion.influenceActionCosts.blessRegion})</span>
              )}
            </button>
            
            <button
              onClick={() => handleInfluenceAction('Corrupt Region', selectedRegion.id, selectedRegion.name, 'region')}
              disabled={
            ...
            >
              ...
              {selectedRegion.influenceActionCosts?.corruptRegion !== undefined && (
                <span className="text-xs"> (Cost: {selectedRegion.influenceActionCosts.corruptRegion})</span>
              )}
            </button>
            
            <button
              onClick={() => handleInfluenceAction('Guide Research in Region', selectedRegion.id, selectedRegion.name, 'region')}
              disabled={
            ...
            >
              ...
              {selectedRegion.influenceActionCosts?.guideResearch !== undefined && (
                <span className="text-xs"> (Cost: {selectedRegion.influenceActionCosts.guideResearch})</span>
              )}
            </button>
          </div>
        </div>
