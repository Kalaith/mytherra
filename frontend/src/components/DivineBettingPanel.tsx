...
import { getDivineBets, getSpeculationEvents, getBettingOdds, placeDivineBet, CreateDivineBetPayload } from '../api/apiService';
              ...
              speculationEvents.map(event => (
                <div key={event.id} className="p-3 bg-gray-700 rounded">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-400 mb-3">
                    <div>Timeframe: {event.timeframe.minimum}-{event.timeframe.maximum} years</div>
                    <div>Options: {event.bettingOptions.length} available</div>
                  </div>

                  {event.bettingOptions.map(option => (
                    <div key={option.id} className="flex justify-between items-center p-2 bg-gray-600 rounded mb-2">
            ...
            {bettingOdds.length === 0 ? (
              ...
              bettingOdds.map(odds => (
                <div key={odds.eventId} className="p-3 bg-gray-700 rounded">
                  ...
                  </div>                  <div className="text-xs text-gray-400 mt-2">
