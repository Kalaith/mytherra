...
const LandmarkList: React.FC<LandmarkListProps> = ({ landmarks, onSelectLandmark }) => {
  if (landmarks.length === 0) {
    return <div className="text-center py-8 text-gray-400">No landmarks found in this region</div>;
  }

  return (
    <div className="space-y-1">
      {landmarks.map(landmark => (
        <LandmarkItem key={landmark.id} landmark={landmark} onSelectLandmark={onSelectLandmark} />
      ))}
    </div>
  );
};
...
const LandmarkItem: React.FC<LandmarkItemProps> = ({ landmark, onSelectLandmark }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center">
        ...
        <div className="bg-gray-800 px-2 py-1 rounded text-xs capitalize">{landmark.status}</div>
      </div>
    </div>
  );
};
