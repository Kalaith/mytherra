      ...
      <div className="space-y-2 max-h-96 overflow-y-auto">
        {settlements.map(settlement => (
          <div
            ...
            onClick={() =>
              onSelectSettlement(selectedSettlement?.id === settlement.id ? null : settlement)
            }
          >
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-gray-400">Prosperity: {settlement.prosperity}%</p>
              </div>
            </div>
              ...
              <div className="mt-2">
                <div className="flex flex-wrap gap-1">
                  {settlement.specializations.map(spec => (
                    <span key={spec} className="px-2 py-1 bg-blue-500 text-xs rounded-full">
                      {spec}
                    </span>
                  ))}
                </div>
      ...
      {selectedSettlement && (
        <div className="mt-4 p-3 bg-gray-700 rounded">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div>
              <span className="text-gray-400">Defensibility:</span>{' '}
              {selectedSettlement.defensibility}%
            </div>
            {selectedSettlement.lastEventYear && (
              <div className="col-span-2">
                <span className="text-gray-400">Last Event:</span> Year{' '}
                {selectedSettlement.lastEventYear}
              </div>
            )}
          </div>
